#!/usr/bin/env python3
"""
ClickHouse DataCache 模块测试脚本 (修复版)
            ...
            for i, config in enumerate(configs, 1):
                try:
                    ...
                    if config['database'] == 'default':
                        # 先连接到默认数据库，然后创建 pulse_trader 数据库
                        import clickhouse_connect
                        client = clickhouse_connect.get_client(**config)
                        logger.info("  ✅ 连接到默认数据库成功")

                        # 创建 pulse_trader 数据库
                        try:
                            client.command('CREATE DATABASE IF NOT EXISTS pulse_trader')
                            logger.info("  ✅ 创建 pulse_trader 数据库成功")
                        except Exception as e:
                            logger.warning(f"  创建数据库警告: {e}")
