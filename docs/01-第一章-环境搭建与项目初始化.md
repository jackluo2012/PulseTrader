...
### 学习目标
- 使用Docker部署ClickHouse数据库
- 配置适合A股量化交易的数据表结构
- 建立Python-ClickHouse连接
- 验证数据库安装和数据操作
...
class ClickHouseManager:
    ...
    def insert_daily_data(self, data: List[Dict]) -> bool:
        ...
        try:
            ...
            logger.info(f"成功插入 {len(insert_data)} 条日线数据")
            return True
    ...
    def get_stock_daily(self, symbol: str, start_date: date,
        ...
        try:
            ...
            logger.info(f"查询到 {len(df)} 条 {symbol} 日线数据")
            return df
...
def test_database_connection():
    ...
    try:
        ...
        logger.info(f"查询到 {len(stock_info)} 只股票")

        # 测试最新价格
        latest_price = db_manager.get_latest_price()
        logger.info(f"查询到 {len(latest_price)} 条最新价格记录")
...
def test_basic_operations():
    ...
    print("\n3. 测试日线数据查询...")
    try:
        # 由于我们还没有真实数据，这个查询会返回空结果
        test_data = db_manager.get_stock_daily(
            symbol='000001.SZ',
            start_date=date(2024, 1, 1),
            end_date=date(2024, 12, 31)
        )
        print(f"✓ 日线数据查询功能正常，返回 {len(test_data)} 条记录")
    except Exception as e:
        print(f"✗ 日线数据查询失败: {e}")
        return False
...
def test_performance():
    ...
    try:
        ...
        print(f"✓ 复杂查询执行时间: {query_time:.4f} 秒")
        print("  市场统计:")
        for market, count, industry_count in result:
            print(f"    {market}: {count} 只股票, {industry_count} 个行业")
        ...
        print(f"✓ Redis缓存查询时间: {cache_time:.4f} 秒")
...
✓ 查询到 4 只股票
  ...
  000001.SZ 平安银行      SZ      银行 1991-04-03        None          1 2024-11-24 12:00:00
...
恭喜！您已经成功完成了ClickHouse数据库的安装和配置。在本节中，您学会了：

1. ✅ 使用Docker部署ClickHouse数据库
2. ✅ 配置A股专用数据表结构
3. ✅ 建立Python-ClickHouse连接池
4. ✅ 实现数据查询和缓存功能
5. ✅ 验证数据库性能和功能

**下一步：** 现在您有了一个高性能的时序数据库，可以在下一节学习如何使用Akshare获取A股数据并存储到ClickHouse中。

---

## 提示

- 保持ClickHouse服务运行状态：`docker-compose up -d`
- 定期检查数据库健康状态：运行测试脚本
- 使用Redis缓存提高查询性能
- 下一步我们将学习如何获取真实的A股数据
